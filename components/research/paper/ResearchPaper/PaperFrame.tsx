...
type PaperFrameProps = {
  /** Path or absolute URL, e.g. "/Research-paper/Jyotish/astrology-of-seers.pdf" */
  src: string;
  type?: "pdf" | "docx";
  title?: string;
  subtitle?: string;
...
export default function PaperFrame({
  ...
  const resolvedType: "pdf" | "docx" = useMemo(() => {
    if (type) return type;
    if (isPdf(src)) return "pdf";
    if (isDocx(src)) return "docx";
    return "pdf";
  }, [src, type]);
  ...
  const call = (fn: (app: any) => void, fallback?: () => void) => {
    try {
      const win = iframeRef.current?.contentWindow as any;
      const app = win?.PDFViewerApplication;
  ...
  const setHashKV = (key: string, val: string) => {
    try {
  ...
  const bumpHash = (key: "zoom" | "rotation", delta: "+" | "-90" | "+90" | "-") => {
    try {
      const w = iframeRef.current?.contentWindow;
      if (!w) return;
      const url = new URL(w.location.href);
  ...
  return (
    <section
      ...
      className={clsx(
        ...
        "rounded-2xl border border-neutral-200/70 bg-white/70 backdrop-blur-md",
        "shadow-[0_10px_40px_rgba(0,0,0,0.06)]",
        "dark:bg-neutral-900/60 dark:border-neutral-800",
        className
      )}
      aria-label={title ?? "Paper viewer"}
    >
      {/* Header / meta */}
        ...
        <div className="flex items-center gap-2">
          ...
          {allowDownload && (
            <button
              ...
              className="rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs font-medium text-neutral-700 shadow-sm transition hover:bg-neutral-50 active:translate-y-px dark:bg-neutral-800 dark:text-neutral-200 dark:border-neutral-700"
      ...
      {resolvedType === "pdf" && (
        <div
          className="flex flex-wrap items-center gap-1.5 border-b border-neutral-200/70 bg-white/70 px-3 py-2 text-xs text-neutral-700 backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-900/60 dark:text-neutral-200"
          role="toolbar"
        ...
        >
          ...
          <span className="px-1">
            <input
              ...
              className="h-7 w-14 rounded border border-neutral-300 bg-white px-2 text-center text-xs outline-none dark:border-neutral-700 dark:bg-neutral-800"
              value={page}
              onChange={(e) => {
      ...
      <div ref={containerRef} className="relative">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 z-[5] rounded-b-2xl"
          style={{
            background:
              "linear-gradient(to bottom, rgba(255,255,255,0.12), transparent 60%, transparent 40%, rgba(255,255,255,0.06))",
          }}
        />
        <iframe
          ref={iframeRef}
          ...
          className="relative z-0 block w-full rounded-b-2xl bg-white dark:bg-neutral-900"
          src={frameUrl}
          title={title ?? "Paper"}

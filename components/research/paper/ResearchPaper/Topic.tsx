...
export default function Topic({ title, author, description, className }: TopicProps) {
  return (
    <motion.section
      itemScope
      ...
      aria-label="Paper topic"
    >
      {/* Title */}
      <h1
        itemProp="headline"
        className="text-[22px] sm:text-[26px] font-semibold tracking-tight text-neutral-900 dark:text-neutral-100"
      >
        {title}
      </h1>
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-x-2 -top-2 h-8 rounded-2xl bg-gradient-to-b from-white/60 to-transparent dark:from-neutral-900/50"
      />
    </motion.section>
  );
}

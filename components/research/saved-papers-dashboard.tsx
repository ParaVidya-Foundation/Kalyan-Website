...
import type { SavedPaper } from "@/lib/dashboard-data";

type SavedPapersDashboardProps = {
...
export function SavedPapersDashboard({ documents }: SavedPapersDashboardProps) {
  ...
  const tags = useMemo(
    () => ["All", ...new Set(documents.flatMap((doc) => doc.tags))],
    [documents]
  );
  ...
  return (
    <DashboardShell
      title="Saved papers"
      description="Research documents, assessments, and ritual notes kept in one calm view."
      rightPanel={rightPanel}
      actions={
        <Button className="rounded-full bg-gray-900 px-5 text-white">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="relative w-full md:w-64 lg:w-72">
            <input
              ...
              placeholder="Search title, type, notes..."
              className="w-full rounded-2xl border border-gray-200 bg-white/90 px-4 py-2 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-gray-900/10"
            />

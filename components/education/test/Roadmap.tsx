...
type StepSection = {
  id: "foundation" | "analysis" | "mastery";
  label: string;
  subtitle: string;
  gradient: string;
  items: StepItem[];
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-28">
        {ROADMAP.map((section) => (
          <motion.div
            ...
            whileInView="show"
            viewport={{ once: true }}
            className="grid lg:grid-cols-3 gap-14 items-start"
          >
            {/* Section Header */}
            <div className="lg:sticky lg:top-32">
              <div
                className={`w-56 h-56 rounded-full bg-gradient-to-br ${section.gradient} flex flex-col items-center justify-center shadow-xl`}
              >
            ...
            <div className="lg:col-span-2 grid sm:grid-cols-2 gap-6">
              {section.items.map((item) => (
                <motion.div key={item.step} variants={card}>

...
const EduSearchBox: React.FC<EduSearchBoxProps> = ({
  placeholder = "Search your Book, Topic or Course...",
  onSearch,
  buttonText = "Start with AI",
  redirectTo = "/education/chat",
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!query) return;
    if (onSearch && !enableRealtimeSearch) onSearch(query);
    if (redirectTo) {
  ...
  return (
    <form
      ...
      className="group relative mx-auto flex w-full max-w-2xl items-center gap-3 rounded-2xl border border-blue-200/60 bg-white/80 p-2 shadow-[0_10px_30px_rgba(59,130,246,0.15)] backdrop-blur-xl transition-all duration-300 hover:shadow-[0_16px_40px_rgba(59,130,246,0.25)]"
      role="search"
    >
      {/* Left Icon */}
      ...
      <input
        ...
        className="peer w-full flex-1 bg-transparent px-2 py-3 text-base text-gray-800 placeholder-gray-400 outline-none sm:text-lg"
      ...
      <button
        type="submit"
        className="relative inline-flex items-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 px-5 py-3 text-sm font-semibold text-white shadow-md transition-all hover:scale-[1.02] sm:px-6 sm:py-3 sm:text-base"
      >

...
export function TestDashboard({ tests }: TestDashboardProps) {
  ...
  const { attemptedTests, newTests } = useMemo(() => {
    const attempted: TestSummary[] = [];
    const fresh: TestSummary[] = [];

    for (const test of tests) {
      const status = (test as any)?.status as string | undefined;
      const attempts = (test as any)?.attempts as number | undefined;
      ...
      if (isAttempted) attempted.push(test);
  ...
  return (
    <DashboardShell
      title="Education Tests"
      description="Track your astro learning, revisit completed tests, and follow a calm, guided roadmap."
      rightPanel={<QuickActionsPanel actions={quickActions} />}
    >
      {/* TOP: Tests + Roadmap */}
      <section className="grid gap-6 lg:grid-cols-[2fr_minmax(260px,1fr)] items-start">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            {newTests.length === 0 ? (
              <div className="rounded-2xl border border-yellow-100 bg-white p-5 text-sm text-gray-500">
                You’ve completed all current modules. New tests will appear here soon.
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                {newTests.map((test) => (
                  <TestCard key={test.id} {...test} onStart={() => startTest(test)} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Badge
                variant="outline"
                className="rounded-full border-gray-200 text-xs text-gray-600"
              >
        ...
        >
            ...
            {[
              ...
              >
                ...
                <span
                  className={cn(
                    "flex-1",
                    step.status === "done" && "line-through decoration-emerald-400/70",
          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            <Button
              variant="outline"
              className="w-full rounded-full border-yellow-200 text-xs text-gray-700"
              onClick={() => startTest(newTests[0] ?? tests[0])}
            >

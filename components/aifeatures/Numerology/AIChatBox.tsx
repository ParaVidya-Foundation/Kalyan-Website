...
const AIChatBox: React.FC<AIChatBoxProps> = ({
  onSend,
  placeholder = "why my destiny number is 8 ?",
  disabled = false,
  isSending = false,
  className = "",
}) => {
  ...
  return (
    <form
      ...
      className={`relative flex w-full items-center gap-3 rounded-2xl border border-white/20 
      ...
      transition-colors transition-shadow duration-200 ${className}`}
      aria-label="Astrology AI chat input"
    >
      <input
      ...
      >
        {sending ? (
          ...
          >
            <path
              d="M3.4 20.6 21 12 3.4 3.4l3.2 7.2 7.2 1.4-7.2 1.4-3.2 7.2Z"
              fill="currentColor"
            />
          </svg>
        )}

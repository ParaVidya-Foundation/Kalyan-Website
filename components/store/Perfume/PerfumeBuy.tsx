...
type Props = {
  ...
  sku: string;
  price: Price;
  sizeOptions: Option[]; // e.g. 30ml, 50ml, 100ml
  fragranceNotes?: string; // e.g. “Woody, Floral, Musky”
  longevity?: string; // e.g. “8-10 hours”
  hallmarks?: string[]; // trust badges
  onAddToCart?: (payload: any) => void;
  onBuyNow?: (payload: any) => void;
};

export default function PerfumeBuy({
  title,
  sku,
  price,
  sizeOptions,
    ...
    <aside className="w-full rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-sm">
      ...
      <div className="mt-5 flex items-center gap-2">
        <input
          ...
          inputMode="numeric"
          maxLength={6}
          className="h-10 w-40 rounded-md border border-neutral-300 px-3 text-sm outline-none focus:border-neutral-600"
          value={pincode}
      ...
      <div className="mt-5">
        ...
        <select
          ...
          className="h-10 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:border-neutral-600"
        >
          {sizeOptions.map((o) => (
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          className="h-11 rounded-md bg-black text-white text-sm font-medium hover:opacity-90 transition"
          onClick={() => onBuyNow?.(payload)}
        >
          BUY NOW
        </button>
        <button
          className="h-11 rounded-md border border-neutral-300 text-sm font-medium hover:bg-neutral-50 transition"

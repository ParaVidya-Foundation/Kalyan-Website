...
export type FeatureItem = {
  title: string;
  subtitle?: string;
  icon?: React.ReactNode; // user-supplied icon or image node
  ariaLabel?: string;
  id?: string | number;
  onClick?: () => void;
};
...
type FeaturesProps = {
  ...
  className?: string;
  animated?: boolean;
  /** number of columns at large screen (default 6) */
  columns?: number;
  /** accent color (svg/icon color & title color) */
  accentColor?: string;
  /** icon container size in px */
  iconSize?: number;
  /** gap between cards (tailwind spacing units in px fallback) */
  gap?: number;
};
...
function PlaceholderIcon({ size = 44 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
...
export default function Features({
  ...
  animated = true,
  columns = 6,
  accentColor = "#B77634", // golden accent (used for icon & title)
  iconSize = 44,
  gap = 24,
}: FeaturesProps) {
  ...
  const itemVariants = {
    ...
    show: { opacity: 1, y: 0, transition: { duration: 0.45, ease: [0.19, 1, 0.22, 1] as [number, number, number, number] } },
  };

  return (
    <section
      aria-label="Why choose us"
      className={`${poppins.className} w-full ${className}`}
    >
      <motion.div
        ...
        >
          {items.map((it, idx) => (
            <motion.button
              ...
              onClick={it.onClick}
              className="flex flex-col items-center text-center gap-2 p-2 bg-transparent border-0 cursor-pointer"
              type="button"
            >
              <div
              ...
              <h3
                className="mt-1 text-[0.95rem] sm:text-sm font-semibold leading-tight"
              ...
              {it.subtitle && (
                <p className="mt-1 text-[0.82rem] text-neutral-500 leading-snug">
                  {it.subtitle}
                </p>
              )}
            </motion.button>

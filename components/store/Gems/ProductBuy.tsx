...
type Props = {
  ...
  sku: string;
  price: Price;
  metalOptions: Option[];          // e.g. 18Kt, 14Kt
  colorOptions: Option[];          // Yellow/White/Rose
  sizeOptions: Option[];           // 9-24 or US sizes
  diamondSpec?: string;            // e.g. "0.237 Ct + 0.15 Ct solitaire"
  hallmarks?: string[];            // e.g. ["BIS Hallmarked", "SGL Certified"]
...
export default function ProductBuy({
  ...
  sizeOptions,
  diamondSpec,
  hallmarks = ["BIS Hallmarked", "IGI/SGL Certified", "30-Day Returns"],
  onAddToCart,
  onBuyNow,
}: Props) {
  ...
  return (
    <aside className="w-full rounded-2xl border border-neutral-200 bg-white p-4 sm:p-6">
      {/* Title + price */}
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        />
        <button
          className="h-10 rounded-md border border-neutral-200 px-3 text-sm hover:bg-neutral-50"
          onClick={() => null}
        >
          Check
        </button>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <button
          className="h-11 rounded-md bg-neutral-900 text-white hover:opacity-90"

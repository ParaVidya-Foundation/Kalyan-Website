...
type GemsDetailProps = {
  ...
  /** optional small badges (like Good Fortune) */
  badges?: string[];
  className?: string;
};
...
export default function GemsDetail({
  specs = defaultSpecs,
  description = (
    <>
  ...
  badges = ["Good Fortune", "Wealth", "New Opportunities", "Business Success"],
  className,
}: GemsDetailProps) {
  return (
    <section
    ...
    >
        ...
        <div className="mb-6">
          <div className="w-full rounded-md bg-neutral-50/80 dark:bg-neutral-900/40 py-3 px-4 flex flex-wrap gap-4 justify-center">
            {badges.map((b, i) => (
              <span
                key={`${b}-${i}`}
                className="inline-flex items-center gap-2 text-[0.95rem] font-medium text-amber-700"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        >
          <div
            className="border border-neutral-200 rounded-md p-6 bg-white/60 dark:bg-neutral-900/40"
            role="region"
            aria-label="Gems specifications"
          >
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
              {specs.map((s, idx) => (
                <div key={`${String(s.label)}-${idx}`} className="min-h-[48px]">
                  <dt
        ...
        >
          <div className="bg-white/60 dark:bg-neutral-900/40 p-6 rounded-md">
            ...
            <div>
              <h4
                className="text-lg font-semibold mb-3 text-amber-800"
                style={{ fontFamily: playfair.className }}
              >
                BENEFITS
              </h4>
              ...
              <div className="mt-6 flex flex-wrap gap-3 items-center">
                <button
                  className="inline-flex items-center gap-2 rounded-md bg-amber-700 px-4 py-2 text-white text-sm shadow-sm hover:shadow-md transition"
                  onClick={() => {
                    if (typeof window !== "undefined") {
                ...
                <button
                  className="inline-flex items-center gap-2 rounded-md border border-amber-700 px-4 py-2 text-amber-700 text-sm hover:bg-amber-50 transition"
                  onClick={() => {
                    if (typeof window !== "undefined") {

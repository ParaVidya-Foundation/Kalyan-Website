...
export type Accessories = {
  ...
  price: number;
  oldPrice?: number;
  showFrom?: boolean; // Show "From ₹" prefix
  href?: string;
};
...
export default function AccessoriesCards({ accessories, className = "" }: Props) {
  const discount =
    accessories.oldPrice && accessories.oldPrice > accessories.price
          ...
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            {accessories.oldPrice && (
              <>
                ...
                {discount && (
                  <span className="text-xs font-semibold text-white bg-gray-900 px-2 py-0.5 rounded">
                    {discount}% OFF
                  </span>
                )}
              </>

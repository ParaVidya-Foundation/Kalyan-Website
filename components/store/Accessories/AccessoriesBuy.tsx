...
type Props = {
  ...
  beadSizeOptions?: Option[];       // 6mm / 8mm / 10mm / 12mm
  lengthOptions?: Option[];         // mala length / bracelet size
  energyOptions?: Option[];         // energization type
  materialOptions?: Option[];       // wood / rudraksha / stone etc.

  certifications?: string[];        // Gov / Lab certified
...
export default function ProductBuy({
  ...
  return (
    <aside className="w-full rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6">
      {/* TITLE */}
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {beadSizeOptions.length > 0 && (
          <Selector
            label="Bead Size"
            value={beadSize}
            onChange={setBeadSize}
      ...
      <div className="mt-4 flex items-center gap-3 text-sm">
        <input
          title="Energize before shipping (Free)"
          type="checkbox"
          checked={energize}
          onChange={(e) => setEnergize(e.target.checked)}
          className="h-4 w-4"
        />
  ...
  return (
    <label className="flex flex-col gap-1 text-sm">
      ...
      <select
        ...
        className="h-10 rounded-md border border-neutral-300 px-3 text-sm outline-none focus:border-neutral-600"
      >
        {options.map((o) => (

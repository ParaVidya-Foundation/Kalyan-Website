...
export default function ImageTrail({
  ...
  spawnEveryPx = 85,
  inertia = 0.07,
  headline = "Where Elegance Follows You.",
  subline = "Every move paints a whisper of luxury — calm, subtle, and alive.",
}: Props) {
  const heroRef = useRef<HTMLDivElement>(null);
  const poolRef = useRef<HTMLDivElement | null>(null);
  const spritesRef = useRef<{ el: HTMLDivElement; img: HTMLImageElement }[]>([]);
  ...
  const updateBounds = useCallback(() => {
    if (heroRef.current) rectRef.current = heroRef.current.getBoundingClientRect();
  }, []);
  ...
  useEffect(() => {
    ...
    const spawnAt = (x: number, y: number) => {
      ...
      if (!rect || !poolRef.current) return;

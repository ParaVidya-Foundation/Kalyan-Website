...
type Price = { mrp: number; sale?: number };

type Props = {
  title: string;
  sku: string;
  price: Price;
  sizes: Option[];             // A4, A3, A2, 12x18, 24x36 etc.
  paperTypes: Option[];        // Glossy, Matte, Textured
  frameOptions: Option[];      // No frame, Black frame, White frame, Wood frame
  finishOptions: Option[];     // Laminated, Non-laminated
  hallmarks?: string[];        // "Premium Print", "Museum Quality", etc.
  onAddToCart?: (payload: any) => void;
  onBuyNow?: (payload: any) => void;
  onCustomPoster?: () => void;
};

export default function PosterBuy({
  title,
  sku,
  price,
  sizes,
  ...
  frameOptions,
  finishOptions,
  hallmarks = ["Premium Art Print", "Fade-Resistant Ink", "Museum Quality"],
  onAddToCart,
  onBuyNow,
  onCustomPoster,
}: Props) {
  ...
  const discount = useMemo(() => {
    if (!price.sale || price.sale >= price.mrp) return 0;
  ...
  return (
    <aside className="w-full rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-sm">
      ...
      <div className="mt-5 flex items-center gap-2">
        <input
          ...
          inputMode="numeric"
          maxLength={6}
          className="h-10 w-40 rounded-md border border-neutral-300 px-3 text-sm outline-none focus:border-neutral-500"
          value={pincode}
      ...
      <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <button
          className="h-11 rounded-md bg-black text-white hover:opacity-90"
        ...
        <button
          className="h-11 rounded-md border border-neutral-300 hover:bg-neutral-50"
        ...
        <button
          className="h-11 rounded-md bg-white border border-neutral-300 hover:bg-neutral-50 sm:col-span-2 flex items-center justify-center gap-2"
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 rounded-lg border border-neutral-200 p-3">
        {hallmarks.map((h, i) => (
          <div key={i} className="text-center text-sm text-neutral-700">
            {h}
          </div>
        ))}
      </div>
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <select
        ...
        className="h-10 rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:border-neutral-500"
      >
        {options.map((o) => (

...
export function KundliChart({ chartData, title, size = "medium" }: KundliChartProps) {
  const sizeClasses = {
    small: "w-32 h-32",
    medium: "w-48 h-48",
    large: "w-64 h-64",
        ...
        <div className="absolute inset-0 grid grid-cols-3 grid-rows-3">
          {chartData.flat().map((value, index) => {
            ...
            return (
              <div
                key={index}
                className={`border border-gray-400 flex items-center justify-center ${cellSizeClasses[size]} font-medium`}
              >
                {value || ""}
              </div>
            )
